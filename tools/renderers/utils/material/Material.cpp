...
namespace Tools { namespace Renderers { namespace Utils { namespace Material {
    ...
    Material::Material(IRenderer& renderer, Luasel::Ref const& type, Luasel::Ref& materialSelf, std::function<IShaderProgram&(std::string const&)> const& loadShader, std::function<std::unique_ptr<Texture::ITexture>(std::string const&)> loadTexture) :
        _renderer(renderer),
