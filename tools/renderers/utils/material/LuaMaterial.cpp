...
namespace Tools { namespace Renderers { namespace Utils { namespace Material {
    ...
    LuaMaterial::LuaMaterial(IRenderer& renderer, Luasel::Ref const& registeredMaterial, std::function<IShaderProgram&(std::string const&)> const& loadShader, std::function<std::unique_ptr<Texture::ITexture>(std::string const&)> loadTexture) :
