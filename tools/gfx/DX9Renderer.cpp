...
namespace Tools { namespace Gfx {
    ...
    std::unique_ptr<IRenderTarget> DX9Renderer::CreateRenderTarget(glm::uvec2 const& imgSize)
    {
    ...
    std::unique_ptr<IProgram> DX9Renderer::CreateProgram(std::string const& vertexCode, std::string const& fragmentCode)
    {
        DX9::VertexProgram vp(*this, vertexCode);
        DX9::FragmentProgram fp(*this, fragmentCode);
        return std::unique_ptr<IProgram>(new DX9::Program(*this, std::move(vp), std::move(fp)));
    }
    ...
    void DX9Renderer::SetCullMode(CullMode::Type type)
    {
        switch (type)
        {
        case CullMode::None: this->_device->SetRenderState(D3DRS_CULLMODE, D3DCULL_NONE); break;
        case CullMode::Clockwise: this->_device->SetRenderState(D3DRS_CULLMODE, D3DCULL_CW); break;
        case CullMode::CounterClockwise: this->_device->SetRenderState(D3DRS_CULLMODE, D3DCULL_CCW); break;
    ...
    void DX9Renderer::Unregister(DX9::Program& program)
    {
    ...
    void DX9Renderer::_PushState(ARenderer::RenderState const& state)
    {
        bool startScene = false;
        if (this->_states.size() > 0)
        {
            if (state.target != 0 && this->_states.size() > 1)
                DXCHECKERROR(this->_device->EndScene());
            startScene = this->_states.size() == 1 || state.target != 0;
        }

        this->_GenericPushState(state, DX9Renderer::glToDirectX);
