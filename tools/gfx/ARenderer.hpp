...
namespace Tools { namespace Gfx {
    ...
    protected:
        ARenderer(glm::uvec2 const& screenSize, bool fullscreen) : _screenSize(screenSize), _fullscreen(fullscreen), _isRenderToTexture(false), _currentProgram(nullptr) {}
    ...
    public:
        ...
        //virtual std::unique_ptr<IProgram> CreateProgram(std::string const& vertex, std::string const& fragment) = 0;
    ...
    protected:
        void _GenericPushState(RenderState const& state, glm::mat4 const& projection = glm::mat4(1.0f))
        {
            ...
            if (this->_states.size() > 0)
                ...
                rsOld.renderToTexture = false;

            this->_states.push_front(state);
            auto& rs = this->_states.front();
            this->_currentState = &rs;

            rs.renderToTexture = rsOld.renderToTexture || rs.target != nullptr;

            if (rs.target != nullptr)
            {
                rs.target->Bind();
                this->SetViewport(glm::uvec2(0, 0), rs.target->GetSize());
            }
            else if (rsOld.target != nullptr)
            {
                rs.target = rsOld.target;
            }
        ...
        bool _GenericPopState()
        {
            ...
            if (rsOld.target != 0 && rsOld.target != rs.target)
            {
                if (rs.target != 0)
                {
                    rs.target->Bind();
