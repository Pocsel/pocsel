    ...
    Tools::MessageQueue<MapGenerator>(this, 4)
{
    ...
    for (auto it = conf.cubes.begin(), ite = conf.cubes.end() ; it != ite ; ++it)
    {
        ...
        // XXX OLD
//        std::cout << "priority" << it->second.priority << "\n";
//        std::cout << "equation" << it->second.equation << "\n";
//        std::cout << "validator" << it->second.validator << "\n";
    ...
    //std::list<MapConf::CubeConf const*> cube_confs; // XXX OLD
...
//    {
//        bool inserted = false;
//        for (auto ccit = cube_confs.begin(), ccite = cube_confs.end() ; ccit != ccite && !inserted ; ++ccit)
//        {
//            if (it->second.priority < (*ccit)->priority)
//            {
//                cube_confs.insert(ccit, &it->second);
//                inserted = true;
//            }
//            else if (it->second.priority == (*ccit)->priority)
//            {
//                cube_confs.insert(ccit, &it->second);
//                inserted = true;
//                std::cout << "WARNING: priority of '" << it->second.type->name <<
//                    "' is the same as '" << (*ccit)->type->name << "'\n        -> the first is '" <<
//                    it->second.type->name << "'\n";
//            }
//        }
//        if (!inserted)
//            cube_confs.push_back(&it->second);
//    }
    ...
    //XXX OLD
//    int cur_equation_idx = 0;
//    for (auto it = cube_confs.begin(), ite = cube_confs.end() ; it != ite ; ++it)
//    {
//        if (equations_done.find((*it)->equation) != equations_done.end())
//        {
//            Validator* validator = validators.CreateValidator((*it)->validator, (*it)->validator_values);
//            if (validator != 0)
//            {
//                int& idx = equations_done[(*it)->equation];
//                if (idx == -1)
//                {
//                    idx = cur_equation_idx++;
//                    this->_equations.push_back(equations[(*it)->equation]);
//                }
//
//                this->_cubes.push_back(new CubeSpawnInfo((*it)->type->id, idx, validator));
//            }
//            else
//            {
//                std::cout << "WARNING: validator '" << (*it)->validator
//                    << "' does not exist. (cube '" << (*it)->type->name
//                    << "' will not be produced)\n";
//            }
//        }
//        else
//        {
//            std::cout << "WARNING: equation '" << (*it)->equation
//                << "' does not exist. (cube '" << (*it)->type->name
//                << "' will not be produced)\n";
//        }
//    }
        ...
        for (z = 0 ; z < Common::ChunkSize ; ++z)
        {
            ...
            for (y = 0 ; y < Common::ChunkSize ; ++y)
            {
                        ...
                        ite = this->_cubes.end() ; it != ite ; ++it)
                {
                    ...
                    // XXX OLD
//                    if (!results_done[(*it)->equationIndex])
//                    {
//                        results_done[(*it)->equationIndex] = true;
//                        results[(*it)->equationIndex] = this->_equations[(*it)->equationIndex]->Calc(xx, yy, zz);
//                    }
//                    if ((*it)->GetValidator().Check(xx, yy, zz, results[(*it)->equationIndex]))
//                    {
//                        nuChunk->SetCube(x, y, z, (*it)->cubeType);
//                        break;
//                    }
